...
<div class="wrapper">
    ...
    <li class="text-info">Downloads<span class="number">${s.downloads}</span>
    </li>
    </ul>
</div>
</li>
`,i=document.querySelector(".js-search-form"),c=document.querySelector(".js-gallery"),y=document.querySelector(".loader"),u=document.querySelector(".js-load-more");let o=1,d="",g=0;const L=new w(".js-gallery a",{overlay:!0,captionsData:"alt",overlayOpacity:.8,captionDelay:250,focus:!0});function b(){y.classList.remove("is-hidden")}function v(){y.classList.add("is-hidden")}const S=async s=>{try{b(),s.preventDefault(),d=i.elements.user_query.value.trim(),o=1;const t=await m(d,o);if(console.log(t),d===""){n.warning({title:"Caution",message:"Input field must not be empty",position:"topLeft"});return}if(t.data.hits.length===0){n.error({title:"Error",message:"Sorry, there are no images matching your search query. Please try again!",position:"bottomRight"}),c.innerHTML="",i.reset();return}const a=t.data.hits.map(r=>f(r)).join("");c.innerHTML=a,g=c.querySelector("li").getBoundingClientRect().height,u.classList.remove("is-hidden"),L.refresh();const e=Math.ceil(t.data.totalHits/15);o>=e&&(u.classList.add("is-hidden"),n.info({title:"Info",message:"We are sorry,but you have reached the end of search results"}))}catch(t){console.log(t)}finally{v()}i.reset()},q=async s=>{try{b(),o++;const t=await m(d,o);console.log(t);const a=t.data.hits.map(e=>f(e)).join("");c.insertAdjacentHTML("beforeend",a),scrollBy({top:g*2,behavior:"smooth"}),L.refresh();const l=Math.ceil(t.data.totalHits/15);o>=l&&(u.classList.add("is-hidden"),n.info({title:"Info",message:"We are sorry,but you have reached the end of search results"}))}catch(t){console.log(t)}finally{v()}i.reset()};i.addEventListener("submit",S);u.addEventListener("click",q);
